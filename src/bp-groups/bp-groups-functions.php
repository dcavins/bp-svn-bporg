 ...
 *         @type array  $capabilities    Singular name.
...
function bp_groups_deregister_group_status( $group_status ) {
	 ...
	 * @param object $status       Group status object.
	 */
	do_action( 'bp_groups_deregister_group_status', $group_status );
...
function bp_groups_add_group_capability( $status, $cap, $grant = true ) {
	$bp = buddypress();
	if ( empty( $status ) || empty( $cap ) || ! isset( $bp->groups->statuses[$status] ) ) {
		return false;
	}
	$bp->groups->statuses[$status]->capabilities[$cap] = $grant;
}

function bp_groups_remove_group_capability( $status, $cap, $grant = true ) {
	$bp = buddypress();
	if ( empty( $status ) || empty( $cap ) || ! isset( $bp->groups->statuses[$status] ) ) {
		return false;
	}
	if ( isset( $bp->groups->statuses[$status]->capabilities[$cap] ) ) {
		unset( $bp->groups->statuses[$status]->capabilities[$cap] );
	}
}
...
function bp_groups_group_status_has_cap( $status, $cap ) {
	...
	/**
	 * Filter which capabilities a group status has.
	 *
	 * @since 2.7.0
	 *
	 * @param array  $capabilities Array of status capabilities.
	 * @param string $cap          Capability name.
	 * @param string $name         Status name.
	 */
	$capabilities = apply_filters( 'bp_groups_group_status_has_cap', $bp->groups->statuses[$status]->capabilities, $cap, $status );
...
/**
 * Check whether the group has a non-falsey value for a capability.
 *
 * Useful for simple, yes-or-no type capabilities.
 *
 * @since 2.7.0
 *
 ...
 *                                 'mod', 'admin', or 'noone'.
 * @param string $group_id         ID of the group to check.
