...
function groups_screen_group_forum() {
	...
	if ( !empty( $topic_slug ) && !empty( $topic_id ) ) {
		...
		if ( !$user_is_banned && !bp_action_variable( 2 ) && isset( $_POST['submit_reply'] ) ) {
			...
			if ( bp_groups_auto_join() && !bp_current_user_can( 'bp_moderate' ) && 'anyone_can_join' == bp_groups_group_has_property( $bp->groups->current_group, 'join_method' ) && !groups_is_user_member( bp_loggedin_user_id(), $bp->groups->current_group->id ) ) {
	...
	// Forum topic does not exist.
		...
		if ( isset( $_POST['submit_topic'] ) && bp_is_active( 'forums' ) ) {
			...
			} elseif ( bp_groups_auto_join() && !bp_current_user_can( 'bp_moderate' ) && 'anyone_can_join' == bp_groups_group_has_property( $bp->groups->current_group, 'join_method' ) && !groups_is_user_member( bp_loggedin_user_id(), $bp->groups->current_group->id ) ) {
				// Auto join this user if they are not yet a member of this group.
				groups_join_group( $bp->groups->current_group->id, bp_loggedin_user_id() );
			}
...
function groups_screen_group_request_membership() {
	...
	if ( 'accepts_membership_requests' != bp_groups_group_has_property( $bp->groups->current_group, 'join_method' ) )
		return false;
...
function groups_screen_group_admin_requests() {
	...
	if ( ! bp_is_item_admin() || 'accepts_membership_requests' != bp_groups_group_has_property( groups_get_current_group(), 'join_method' ) ) {
		return false;
	}
