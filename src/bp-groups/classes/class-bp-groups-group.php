...
class BP_Groups_Group {
	...
	protected static function get_hidden_group_ids() {
		...
		// if ( bp_current_user_can( 'bp_manage' ) ) {
		// 	return array();
		// }

		$cache_key = 'hidden_groups_for_user_' . bp_loggedin_user_id();

		$cached = bp_core_get_incremented_cache( $cache_key, 'bp_groups' );
		if ( false === $cached ) {
			...
			// echo PHP_EOL . "statuses: ";
			// var_dump($statuses);

			// Alternatively, fetch groups that have restricted visibility and do bulk comparisons
			/// possible access conditions:
			// 'noone' : These are out!
			// 'admin', 'mod', 'member' : must be a member
			// 'invited' : must be invited
			// 'loggedin' :
			// 'anyone' : publicly visible
			$exclude = array();
			$member = array();
			$invited = array();
			$loggedin = array();
			...
			foreach ( $statuses as $k => $status ) {
				...
				// var_dump( $properties );
				// var_dump( $properties['show_group'] );

				if ( ! is_array( $properties['show_group'] ) ) {
					$properties['show_group'] = preg_split( '/[\s,]+/', $properties['show_group'] );
				}

				if ( in_array( 'noone', $properties['show_group'] ) ) {
					$exclude[] = $k;
				}

				if ( in_array( 'member', $properties['show_group'] ) ) {
					$member[] = $k;
				}

				if ( in_array( 'invited', $properties['show_group'] ) ) {
					$invited[] = $k;
				}

				if ( in_array( 'loggedin', $properties['show_group'] ) ) {
					$loggedin[] = $k;
				}
			...
			$exclude_group_ids = array();
			if ( $exclude ) {
				$def_hidden = groups_get_groups( array( 'status' => $exclude ) );
				$exclude_group_ids = wp_list_pluck( $def_hidden, 'id' );
			}

			if ( $member ) {
				$nonmember_hidden = groups_get_groups( array( 'status' => $member ) );
				// Get user memberships
				// then array diff the two.
			}

			if ( $invited ) {
				$invitee_hidden = groups_get_groups( array( 'status' => $invited ) );
				// Get user invites
				// then array diff the two.
			}
			...
			if ( ! is_user_logged_in() && $loggedin ) {
				$logged_in_only = groups_get_groups( array( 'status' => $loggedin ) );
			}

			// Then join up and array_unique all the excludes.
			// array_merge( $exclude_group_ids, ... );


			bp_core_set_incremented_cache( $cache_key, 'bp_groups', $hidden_group_ids );
		} else {
			$hidden_group_ids = $cached;
		}
